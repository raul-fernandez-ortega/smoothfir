...
void SfRun::print_debug(void)
{
  ...
  if (sfconf->synched_write) {
	...
	printf("    %" PRIu64 "\tcall select fdmax %d\n", (ull_t)((D.o[n].d[i].select.ts_call - D.ts_start) / tsdiv),D.o[n].d[i].select.fdmax);
	printf("    %" PRIu64 "\tret %d\n", (ull_t)((D.o[n].d[i].select.ts_ret - D.ts_start) / tsdiv), D.o[n].d[i].select.retval);
	...
	printf("    %" PRIu64 "\tret %d\n\n", (ull_t)((D.o[n].d[i].write.ts_ret - D.ts_start) / tsdiv), D.o[n].d[i].write.retval);
      }
      if (n == 0) {
	printf("    %" PRIu64 "\tcall synch input trigger start (write)\n", (ull_t)((D.o[n].d[0].init.ts_synchfd_call - D.ts_start) / tsdiv));
	printf("    %" PRIu64 "\tret\n\n", (ull_t)((D.o[n].d[0].init.ts_synchfd_ret - D.ts_start) / tsdiv));
      }
    }
  } else {
    printf("output_process:\n");
  ...
  for (n = 0; n < D.periods && n < DEBUG_RING_BUFFER_SIZE - 2; n++) {
    ...
    if (n == 0) {
      printf("    %" PRIu64 "\tcall init start\n", (ull_t)((D.i[n].d[0].init.ts_start_call - D.ts_start) / tsdiv));
      printf("    %" PRIu64 "\tret\n", (ull_t)((D.i[n].d[0].init.ts_start_ret - D.ts_start) / tsdiv));
    }
    for (i = 0; i < (uint32_t)D.i[n].dai_loops && i < DEBUG_MAX_DAI_LOOPS; i++) {
      printf("    %" PRIu64 "\tcall select fdmax %d\n", (ull_t)((D.i[n].d[i].select.ts_call - D.ts_start) / tsdiv), D.i[n].d[i].select.fdmax);
      printf("    %" PRIu64 "\tret %d (%" PRIu64 ")\n", (ull_t)((D.i[n].d[i].select.ts_ret - D.ts_start) / tsdiv),
	     D.i[n].d[i].select.retval, (ull_t)((D.i[n].d[i].select.ts_ret - D.ts_start) / tsdiv - (D.i[n].d[i].select.ts_call - D.ts_start) / tsdiv));
      printf("    %" PRIu64 "\tread(%d, %p, %d, %d)\n", (ull_t)((D.i[n].d[i].read.ts_call - D.ts_start) / tsdiv), D.i[n].d[i].read.fd,
	     D.i[n].d[i].read.buf, D.i[n].d[i].read.offset, D.i[n].d[i].read.count);
      printf("    %" PRIu64 "\tret %d\n\n", (ull_t)((D.i[n].d[i].read.ts_ret - D.ts_start) / tsdiv), D.i[n].d[i].read.retval);
    }
    ...
    printf("    %" PRIu64 "\tret (%" PRIu64 " from synch input ret)\n\n", (ull_t)((D.f[n].w_output.ts_ret - D.ts_start) / tsdiv),
	   (ull_t)((D.f[n].w_output.ts_ret - D.ts_start) / tsdiv - (D.f[n].r_input.ts_ret - D.ts_start) / tsdiv));
    ...
    printf("    %" PRIu64 "\tret (%lld)\n", (ull_t)((D.o[n].r_filter.ts_ret - D.ts_start) / tsdiv),
	   (ll_t)((D.o[n].r_filter.ts_ret - D.ts_start) / tsdiv - (D.o[n].r_filter.ts_call - D.ts_start) / tsdiv));
    printf("    %" PRIu64 "\tcall synch input %d (write)\n", (ull_t)((D.o[n].w_input.ts_call - D.ts_start) / tsdiv), n - 1);
    printf("    %" PRIu64 "\tret (%lld)\n\n", (ull_t)((D.o[n].w_input.ts_ret - D.ts_start) / tsdiv),
	   (ll_t)((D.o[n].w_input.ts_ret - D.ts_start) / tsdiv - (D.i[n-2].r_output.ts_ret - D.ts_start) / tsdiv));
    if (!sfconf->synched_write && n == 2) {
      printf("    %" PRIu64 "\tcall init start\n", (ull_t)((D.o[n].d[0].init.ts_start_call - D.ts_start) / tsdiv));
      printf("    %" PRIu64 "\tret\n", (ull_t)((D.o[n].d[0].init.ts_start_ret - D.ts_start) / tsdiv));
    }
    for (i = 0; i < (uint32_t)D.o[n].dai_loops && i < DEBUG_MAX_DAI_LOOPS; i++) {
      printf("    %" PRIu64 "\tcall select fdmax %d\n", (ull_t)((D.o[n].d[i].select.ts_call - D.ts_start) / tsdiv), D.o[n].d[i].select.fdmax);
      printf("    %" PRIu64 "\tret %d\n", (ull_t)((D.o[n].d[i].select.ts_ret - D.ts_start) / tsdiv), D.o[n].d[i].select.retval);
	...
	k = D.o[n-1].dai_loops - 1;
      ...
      printf("    %" PRIu64 "\twrite(%d, %p, %d, %d) (%" PRIu64 ")\n", (ull_t)((D.o[n].d[i].write.ts_call - D.ts_start) / tsdiv),
	     D.o[n].d[i].write.fd, D.o[n].d[i].write.buf, D.o[n].d[i].write.offset, D.o[n].d[i].write.count,
	     (ull_t)((D.o[n].d[i].write.ts_call - D.ts_start) / tsdiv - (D.o[n-1].d[k-1].write.ts_call - D.ts_start) / tsdiv));
      printf("    %" PRIu64 "\tret %d\n\n", (ull_t)((D.o[n].d[i].write.ts_ret - D.ts_start) / tsdiv), D.o[n].d[i].write.retval);
    }
    n -= 2; 
  }
