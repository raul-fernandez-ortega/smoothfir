...
static void check_avail(size_t alloc)
{
#ifdef __OS_LINUX__
  ...
  if (fill > 90) {
    fprintf(stderr, "Too much (%d%%) of the available memory is allocated, exiting\n", fill);
    EXIT_PROGRAM;
  }
#endif    
}
...
emallocaligned(size_t size)
{
  ...
  if (ALIGNMENT > size) {
    fprintf(stderr, "ALIGNMENT (%d) is larger than the pagesize (%d), aborting.\n  Recompile with a smaller alignment.\n",(int)ALIGNMENT, (int)getpagesize());
    EXIT_PROGRAM;
  }
  p = malloc(size);
  err = !p;
