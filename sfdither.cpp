 ...
 */
static inline uint32_t tausrand(uint32_t state[3]) 
{
...
static void tausinit(uint32_t state[3], uint32_t seed)
{
  /* default seed is 1 */
  if (seed == 0) {
    seed = 1; 
		 ...
		 vector<struct dither_state*> dither_states)
{
  ...
  realsize = _realsize;
  minspacing = (MIN_RANDTAB_SPACING * sample_rate > max_samples_per_loop) ? MIN_RANDTAB_SPACING * sample_rate : max_samples_per_loop;
  ...
  tausinit(state, 0);
  dither_randtab = (int8_t*)emallocaligned(dither_randtab_size);
  ...
  if (realsize == 4) {
    ...
    for (n = -255; n < 254; n++) {
      ((float *)dither_randmap)[n] = 0.5 + 1.0 / 255.0 + 1.0 / 255.0 * (float)n;
    }
    ((float *)dither_randmap)[254] = 1.5;
  } else {
    ((double *)dither_randmap)[-256] = -0.5;
    for (n = -255; n < 254; n++) {
      ((double *)dither_randmap)[n] = 0.5 + 1.0 / 255.0 + 1.0 / 255.0 * (double)n;
    }
    ((double *)dither_randmap)[254] = 1.5;
  }
  
  for (n = 0; n < n_channels; n++) {
    dither_states.push_back(new struct dither_state());
